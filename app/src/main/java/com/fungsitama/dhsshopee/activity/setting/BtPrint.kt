...
import com.fungsitama.dhsshopee.util.PrinterCommands
import com.fungsitama.dhsshopee.util.Utils
...
class BtPrint(
        ...
        private var LoadingInfo: TextView,
        private var printInfo: TextView,
        private var printButton: Button,
        //private var printthread: Thread
    ...
    private var QRCODE_IMAGE_HEIGHT = 255
    private var QRCODE_IMAGE_WIDTH = 255
    // Other initializer
    ...
    private fun preCheckDone() {
        ...
        LoadingInfo.visibility = View.INVISIBLE
        printButton.isClickable = true
        printSwitch.alpha = 1f

    }
    fun printJudul(stringToPrint: String, keepSocket: Boolean = false) {
    ...
    fun printAirlineCode(stringToPrint: String, keepSocket: Boolean = false) {
        socket.outputStream.write(stringToPrint.toByteArray())
    ...
    fun printAirlineName(stringToPrint: String, keepSocket: Boolean = false) {
        socket.outputStream.write(stringToPrint.toByteArray())
    ...
    fun printSequeceId(stringToPrint: String, keepSocket: Boolean = false) {
        ...
        try {
            ...
            val bitMatrix = qrWriter.encode(stringToPrint, BarcodeFormat.QR_CODE, QRCODE_IMAGE_WIDTH, QRCODE_IMAGE_HEIGHT, hints)
            ...
            for (y in 0 until bitMatrixHeight) {
                ...
                for (x in 0 until bitMatrixWidth) {
                    val i = offset + x
                    if (bitMatrix[x, y]) {
                        color = ContextCompat.getColor(context, R.color.QRCodeBlackColor)
                        //Color.parseColor("#000000")
                        //   ContextCompat.getColor(context,R.color.QRCodeBlackColor)
                        //getResources().getColor(R.color.QRCodeBlackColor)
                    } else {
                        color = ContextCompat.getColor(context, R.color.QRCodeWhiteColor)
                        //Color.parseColor("#ffffff")
                        //ContextCompat.getColor(context, R.color.QRCodeWhiteColor)
            ...
            //bmOverlay = Bitmap.createBitmap(bitmap.width, bitmap.height, bitmap.config)
            val canvas: Canvas = Canvas(bitmap)
            canvas.drawBitmap(bitmap, Matrix(), null)
           // if (bitmap != null) {
                val command = Utils.decodeBitmap(bitmap)
                socket.outputStream.write(PrinterCommands.ESC_ALIGN_CENTER)
    ...
    fun printSmuNumber(stringToPrint: String, keepSocket: Boolean = false) {
        socket.outputStream.write(stringToPrint.toByteArray())
    ...
    fun printAirportOrigin(stringToPrint: String, keepSocket: Boolean = false) {
        socket.outputStream.write(stringToPrint.toByteArray())
    ...
    fun printAirportDestination(stringToPrint: String, keepSocket: Boolean = false) {
        socket.outputStream.write(stringToPrint.toByteArray())
